...
x<-c("biomformat","car","dada2","decontam","DECIPHER","DESeq2","devtools",
     "DirichletMultinomial","dplyr","exactRankTests","FSA","gapminder",
     "GenomicRanges","gganimate","ggplot2","ggradar","ggpubr","gifski","grid",
     "gridExtra","Hmisc","httr","igraph","lme4","lmerTest","magrittr","MASS",
     "Matrix","microbiome","multcomp","nlme","openxlsx","pbkrtest","phangorn",
     "pheatmap","phyloseq","picante","plotly","plyr","png","randomForest",
     "RColorBrewer","Rcpp","RcppArmadillo","RCurl","reshape","reshape2","scales",
     "slam","tibble","tidyr","vegan","rcompanion","leaflet","mediation","bestNormalize",
     "patchwork")
...
######################################################################################
# LOAD asv TABLE
asv<-read.csv("otu.dada2.f230.r200.csv",row.names=1,header=T)
dim(asv) # 371 1268
colnames(asv)[1:5]
row.names(asv)
...
taxa<-read.csv("tax.dada2.f230.r200.csv",header=T,row.names=1)
dim(taxa) # 1702 7
colnames(taxa)
row.names(taxa)[1:5]
...
both<-intersect(row.names(asv),row.names(CHILD));length(both) #198
...
model<-adonis(comm.vst.blind.Mat~cluster, permutations=999);model
...
# Compute Shannon Index with Vegan
x<-vegan::diversity(otu_table(asv, taxa_are_rows=FALSE), index="shannon", MARGIN=1, base=exp(1))
CHILD$alphadiv<-x
hist(CHILD$alphadiv)
shapiro.test(CHILD$alphadiv)
# Non parametric
...
x<-estimate_richness(otu_table(asv, taxa_are_rows=FALSE),measures="Chao1")
CHILD$chao1<-x[,1]
hist(log(CHILD$chao1))
kruskal.test(chao1~cluster, data=CHILD)
dunnTest(data=CHILD,chao1~cluster, method="bh")

alpha_diversity <- estimate_richness(otu_table(asv, taxa_are_rows=FALSE), measure = c("Shannon", "Observed","Chao1"))
alpha_diversity
...
x1<-vegan::diversity(otu_table(asv1, taxa_are_rows=FALSE), index="shannon", MARGIN=1, base=exp(1))
...
lbs$label_ypos<-as.numeric(as.character(c(sum(tb[,1]),sum(tb[,2]),sum(tb[,3]),sum(tb[,4])+1.5,"NA",tb[2,2],tb[2,3],tb[2,4])))
summary(lbs)
...
lbs<-as.data.frame(lbs)
lbs$freq<-as.numeric(as.character(lbs$freq))
lbs$label_ypos<-as.numeric(as.character(c(tb[2,1],tb[2,2],tb[2,3],tb[2,4],sum(tb[,1]),sum(tb[,2]),sum(tb[,3]),sum(tb[,4]))))
summary(lbs)
lbs$csec1<-NA
...
lbs[,2]<-c(rep(levels(CHILD$BF_3m_status)[1],4),rep(levels(CHILD$BF_3m_status)[2],4),rep(levels(CHILD$BF_3m_status)[3],4))
lbs[,3]<-c(tb[1,],tb[2,],tb[3,])
colnames(lbs)<-c("cluster","BF_3m_status","freq")
lbs<-as.data.frame(lbs)
lbs$freq<-as.numeric(as.character(lbs$freq))
lbs$label_ypos<-as.numeric(as.character(c(sum(tb[,1]),sum(tb[,2]),sum(tb[,3]),sum(tb[,4]),sum(tb[3:2,1]),sum(tb[3:2,2]),sum(tb[3:2,3]),sum(tb[3:2,4]),tb[3,1],tb[3,2],tb[3,3],tb[3,4])))
summary(lbs)
...
model_test2<-adonis(vst.blind.Mat2 ~ race_mom4+BF_3m_status+Mother_abs_birth_yn+csec+
                     older_sibs3+infant_sec+AS_bev4, data=Cluster2);model_test2
...
full.model12M <- lm(BMIz_1y~MDS1_trans+MDS2_trans,data=CHILD12M);summary(full.model12M)
Anova(full.model12M,type="III")
confint(full.model12M)
full.model12M <- lm(BMIz_1y~AS_bev4+MDS1_trans,data=CHILD12M);summary(full.model12M)
Anova(full.model12M,type="III")
confint(full.model12M)
